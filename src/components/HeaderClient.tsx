...
// accountId приходит из Header.tsx (серверная обёртка, читает
// getStoredAccountId() через cookie) — сам этот компонент клиентский
// (usePathname, выпадающее меню), поэтому не может прочитать httpOnly
// cookie сессии самостоятельно.
export default function HeaderClient({ accountId }: { accountId: string | null }) {
    ...
    <header className={styles.header}>
      <div className={`container ${styles.inner}`}>
        ...
        <div className={styles.actions}>
          ...
          {/* "Подключить команду" реально запускает OAuth только при
              активной сессии — анонимным посетителям вместо этого показываем
              призыв зарегистрироваться/войти (см. OpenAuthButton.tsx), а не
              кнопку, ведущую в тупик (см. чат про доступ к подключению
              команды). */}
          {!isCabinet &&
            (accountId ? (
              ...
              <OpenAuthButton className={`btnPrimary ${styles.headerCta}`}>
