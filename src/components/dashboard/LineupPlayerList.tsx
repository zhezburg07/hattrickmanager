...
// ВРЕМЕННО упрощено (см. чат "Временно упростить отображение позиционного
// рейтинга") — та же формула, что и число на занятом слоте поля
// (estimateSimpleSlotPotential, см. computePlayerPotential в zoneRatings.ts),
// лучшая из подходящих ролей позиционной группы игрока. Дублируется в
// SquadTable.tsx — тот же текст, тот же принцип "один-в-один", что и у
// baseColumns ниже. Полная формула с калибровкой по реальным матчам
// вернётся, когда накопится достаточно данных.
const POTENTIAL_HINT =
  "Ориентировочная оценка (упрощена на время накопления данных для калибровки): главный навык подходящей позиции + бонус формы, лучшая из подходящих позиций слота.";
