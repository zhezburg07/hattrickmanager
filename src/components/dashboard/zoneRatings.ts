...
// Коэффициенты-потолки (17 у навыка, 3 у формы, сумма — те же 20, что и
// раньше) — навык больше не входит ЛИНЕЙНО, а возводится в куб (см. чат
// "Уточнение по упрощённой формуле — реальный диапазон 9-13"). Причина: с
// чисто линейной формулой (skillFrac*A + formFrac*B, A+B=20) удержать
// потолок ровно в точке навык=20/форма=8 и одновременно посадить топовых,
// но не абсолютно максимальных игроков (навык ~15-17 — "колоссально"/
// "сказочно", а не "легендарно"/"божественно" 19-20, которые практически
// никогда не встречаются) в диапазон 9-13 звёзд — МАТЕМАТИЧЕСКИ невозможно:
// при навыке=16 (80% от 20) и форме=6 (75% от 8) итог A*0.8+B*0.75 лежит в
// [15,16] при ЛЮБОМ неотрицательном разбиении A+B=20, что уже выше желаемого
// диапазона. Реальные данные (составы Hattrick Masters и Отбора ЧМ —
// стабильно 9-13 звёзд даже у игроков мирового уровня) показывают, что
// настоящая шкала Hattrick сама нелинейна: путь от "хорошо" до "мирового
// класса" даёт намного больше прироста оценки, чем путь от "мирового
// класса" до "легендарно". mainSkill^3 сохраняет крайние точки (0→0,
// 20→17, то есть потолок 20 всё ещё достижим ровно при навыке=20 и
// форме=8), но сильно сжимает всё, что ниже абсолютного максимума: навык
// "колоссально"(15)+форма "хорошо"(6) → ≈9.4, навык "сказочно"(17)+форма
// "хорошо"(7) → ≈13.1 — прямо в наблюдаемом диапазоне Masters/Отбора ЧМ.
// 15+ становится достижимо только от навыка 18+ ("волшебно" и выше) —
// действительно исключительный уровень. Форма оставлена линейной — её
// вклад минорный (максимум +3 против +17 у навыка), и, в отличие от
// навыка, форма 7-8 у топ-команд встречается регулярно, а не только у
// легенд игры. Только нижняя граница (0) — верхнего клэмпа по-прежнему нет.
const MAIN_SKILL_MAX_BONUS = 17;
const MAIN_SKILL_CURVE_EXPONENT = 3; // сжимает диапазон — см. комментарий выше
...
export function estimateSimpleSlotPotential(player: Pick<RatingInputs, "skills" | "form">, role: SlotRole): number {
  ...
  const skillFraction = Math.max(0, Math.min(1, mainSkill / 20));
  const base = Math.pow(skillFraction, MAIN_SKILL_CURVE_EXPONENT) * MAIN_SKILL_MAX_BONUS;
  const formBonus = (player.form / 8) * FORM_MAX_BONUS;
