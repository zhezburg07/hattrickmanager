...
//
// СВЕРЕНО с официальным текстом (docs/hattrick-official-rules.md, раздел
// "Игроки: Навыки" — см. чат "Три подтверждённых шага", подтверждено Koke
// как PM перед изменением, п.2 предыдущего аудита):
//   - GK: официальный текст отдельно называет "навык стандартов помогает
//     ему при защите ворот при стандартных положениях" — setPieces раньше
//     отсутствовал вообще; вес 0.5 (тот же шаг, что и у остальных
//     "ситуативных" весов в этой таблице) — та же формулировка "помогает",
//     что и у defending (уже 0.5), не выделена текстом как более важная.
//   - DEF_CENTRAL: "полузащита может являться ОЧЕНЬ ПОЛЕЗНЫМ дополнением" —
//     midfield раньше отсутствовал вообще, добавлен вес 1.2 (между
//     defending-primary и ситуативным passing).
//   - DEF_WIDE: "НЕБОЛЬШОЙ вклад в полузащиту" — midfield раньше
//     отсутствовал, добавлен вес 0.5 (явно МЕНЬШЕ, чем у DEF_CENTRAL — там
//     "очень полезное", здесь "небольшое").
//   - MID_WIDE: "навык полузащиты ТАКЖЕ является НЕОБХОДИМЫМ" — сильная
//     формулировка, близкая к primary (winger); midfield раньше
//     отсутствовал, добавлен вес 2.
//   - FWD_CENTRAL/FWD_WIDE: "полезными являются навыки паса, ФЛАНГА И
//     ПОЛУЗАЩИТЫ" — три навыка перечислены текстом без разделения по
//     важности; midfield раньше отсутствовал у обеих ролей (добавлен 1), у
//     FWD_CENTRAL заодно выровнен winger (было 0.5, вдвое ниже passing без
//     основания в тексте — теперь 1, наравне с passing).
//   - MID_CENTRAL: "ЗНАЧИТЕЛЬНО выигрывают от наличия СУЩЕСТВЕННЫХ навыков
//     паса И защиты" — оба введены одним предложением с одинаковой силой
//     формулировки; было passing=1.5/defending=0.8 (почти вдвое), сближено
//     до 1.1/1.1 (вариант "полное равенство", подтверждён Koke).
//
// ВАЖНО: официальный текст даёт только словесное ранжирование
// ("основным"/"полезным дополнением"/"небольшой вклад"/"необходимым"), а не
// числа — конкретные веса ниже это перевод словесного ранга в уже
// существующую шкалу проекта (шаг 0.5), а не официальная формула Hattrick.
//
// Версия формулы намеренно увеличена (RATING_FORMULA_VERSION ниже) — эта
// правка сбрасывает накопленную калибровку (getAllRoleCalibrations в
// matchRolePredictionsDb.ts), принято сознательно (см. чат).
const slotRoleWeights: Record<SlotRole, Array<[keyof SquadSkills, number]>> = {
  GK: [["goalkeeping", 4], ["defending", 0.5], ["setPieces", 0.5]],
  DEF_WIDE: [["defending", 3], ["winger", 1.5], ["passing", 1], ["midfield", 0.5]],
  DEF_CENTRAL: [["defending", 3.5], ["midfield", 1.2], ["passing", 0.8]],
  MID_WIDE: [["winger", 3], ["midfield", 2], ["passing", 1.5], ["defending", 0.8], ["scoring", 0.5]],
  MID_CENTRAL: [["midfield", 3], ["passing", 1.1], ["defending", 1.1], ["scoring", 0.5]],
  FWD_CENTRAL: [["scoring", 3], ["passing", 1], ["winger", 1], ["midfield", 1]],
  FWD_WIDE: [["scoring", 2.5], ["winger", 1.5], ["passing", 1], ["midfield", 1]],
};
...
// цена, датасет пока маленький (~месяц истории). УВЕЛИЧЕНО с "2" на "3" при
// сверке slotRoleWeights с официальным текстом (см. чат "Три подтверждённых
// шага" — добавлен midfield у DEF_CENTRAL/DEF_WIDE/MID_WIDE/FWD_CENTRAL/
// FWD_WIDE, setPieces у GK, сближены passing/defending у MID_CENTRAL) —
// сброс 43-точечной калибровки принят Koke сознательно, сбор начинается
// заново под более точной базовой формулой (см. PRELIMINARY_CALIBRATION в
// matchRolePredictionsDb.ts — её коэффициенты подобраны под ПРЕЖНЮЮ версию
// "2" и теперь лишь приближение, пока не накопится своя выборка под "3").
export const RATING_FORMULA_VERSION = "3";
...
// applyCalibration). Игрок вне поля не привязан к конкретному слоту (фланг/
// центр), поэтому берём МАКСИМУМ среди всех ролей слота его позиционной
// группы — "какой лучший рейтинг он в принципе способен дать где-то в этой
// линии прямо сейчас". Калибровка применяется ОТДЕЛЬНО к каждой роли (у
// фланга и центра разные коэффициенты), а максимум берётся уже среди
// откалиброванных чисел — не наоборот, иначе максимум сырых рейтингов мог бы
// выбрать роль, которая на самом деле после калибровки даёт результат похуже.
//
// ВОЗВРАЩЕНО (см. чат "Переключить отображение обратно на полную формулу")
// — временное упрощение (главный навык роли + бонус формы, см. историю
// файла) досрочно снято: getAllRoleCalibrations теперь всегда возвращает
// коэффициенты для каждой роли — либо настоящую регрессию (15+ реальных
// матчей), либо временную предварительную заглушку (isPreliminary=true, см.
// PRELIMINARY_CALIBRATION в matchRolePredictionsDb.ts), которая уже даёт
// результаты, близкие к реальным звёздам — искусственно сжатая версия
// больше не нужна.
