...
import MatchesTabs from "@/components/dashboard/MatchesTabs";
...
// фильтр → мягкий фильтр), снятое во время синхронизации. Выключено (см. чат
// "Диагностика на вкладке Матчи видна на проде") — список уже стабильно
// показывает реальные матчи, панель случайно осталась включённой и была
// видна обычным пользователям. Код не удалён — вернуть можно одной строкой,
// если снова понадобится диагностировать конвейер.
const SHOW_MATCHES_DEBUG = false;
    ...
    <>
      ...
      <main className={styles.page}>
        <div className={`container ${styles.stack}`} style={{ paddingBottom: 72 }}>
          <MatchesTabs official={officialContent} arena={arenaContent} />
        </div>
      </main>
      <Footer />
    </>
