...
// ladderdetails.xml — ПОДТВЕРЖДЕНО ОКОНЧАТЕЛЬНО живым запросом по реальному
// LadderId (см. чат "Ещё одна честная проверка ladderdetails.xml"):
// для всех 3 лестниц пользователя (Kazakhstan/Все/Ulytau) файл отдаёт только
// таблицу позиций команд (TeamId/TeamName/Position/Wins/Lost), без единого
// элемента в Matches.Match — список отдельных сыгранных матчей лестницы
// (соперник/счёт/дата) через CHPP действительно недоступен ни в каком виде.
// Расследование закрыто, диагностический запрос этого файла на каждой
// синхронизации убран (см. src/lib/chppSync.ts) — 3 лишних запроса без
// какой-либо пользы для уже закрытого вопроса.
