...
// НАЙДЕННЫЙ баг: все запросы к players.xml в приложении шли без явной версии,
// поэтому requestChppXmlRaw подставлял общий дефолт "1.5" — устаревшую схему
// без Loyalty/MotherClubBonus/Agreeability/Aggressiveness/Honesty и т.д. Имена
// полей ниже были угаданы верно (сверено со сторонним CHPP-клиентом
// github.com/lucianoq/hattrick, chpp/file_players.go — те же поля на том же
// верхнем уровне Player), просто CHPP на версии 1.5 их не отдаёт вовсе. Все
// вызовы players.xml должны указывать эту версию явно.
export const PLAYERS_XML_VERSION = "2.8";
...
//   читаются полями Loyalty/MotherClubBonus — имена полей сверены со сторонним
//   CHPP-клиентом (см. PLAYERS_XML_VERSION выше), но CHPP отдаёт их только на
//   версии 2.8, поэтому раньше колонка молча пропадала (см. hasLoyalty в
//   SquadTable.tsx — не пустой прочерк, а полное скрытие столбца).
...
export function parsePlayersDetailedXml(
  ...
  return players.map((p) => {
    ...
    // Преданность клубу и статус "воспитанника" (сердце) — Loyalty числовое
    // поле на той же шкале 0-20, что и скиллы; для "сердца" перебираем
    // несколько правдоподобных названий булевого поля, из которых точно
    // подтверждено только MotherClubBonus (см. PLAYERS_XML_VERSION выше). Если
    // Hattrick называет остальные иначе — просто останутся undefined и не
    // покажутся, ничего не сломав.
