...
// подтверждённая часть. confirmed=false НЕ означает "ненадёжно" — строгое
// вычитание из уже подтверждённых официальных сумм даёт точный результат
// (см. ZoneCell/ZoneLcrCell в MatchDetailAnalysis.tsx — по запросу число
// показывается визуально как обычное, без курсива/значков, различие видно
// только во всплывающей подсказке при наведении).
...
function debugEventTypeBreakdown(
  ...
  // ИСПРАВЛЕНО (см. чат "П: официально 1, по EventList 0 — код 142 не то,
  // чем казался") — ⚡совпадение выше проверяет только числовое совпадение
  // счётчика с официальным итогом (могло совпасть случайно, особенно при
  // маленьких числах вроде 1 — именно так код 142, реально классифицированный
