...
// ВАЖНО (см. чат "Три подтверждённых шага" — slotRoleWeights сверены с
// docs/hattrick-official-rules.md, RATING_FORMULA_VERSION увеличена
// "2"→"3"): числа ниже подобраны на данных formula_version='2', ДО этой
// сверки весов. Формула изменилась (добавлен midfield у 5 ролей, setPieces
// у GK, сближены passing/defending у MID_CENTRAL) — сырой предсказанный
// рейтинг (predicted_raw) для тех же навыков теперь численно другой, так
// что slope/intercept ниже для версии "3" лишь ПРИБЛИЖЕНИЕ (тот же общий
// порядок величины, раз веса менялись не радикально), а не всё ещё точная
// калибровка — пока свежая выборка под formula_version='3' не накопится и
// не заменит её тем же автоматическим механизмом ниже. Сброс принят Koke
// сознательно, сбор начинается заново.
//
// ВРЕМЕННОЕ ручное приближение (см. чат "Калибровка позиционного рейтинга
// по реальным звёздам Hattrick", план в .claude/plans; обновлено в чате
// "Пересмотр калибровки: занижен/завышен прогноз на 8 игроках", затем в
// чате "43 точки — объединённая регрессия") — пока ни у одной роли нет
// 15+ реальных матчей, чтобы getAllRoleCalibrations честно посчитала
// регрессию по роли сама. Источник ЭТОГО значения: полная выгрузка
// match_role_predictions (formula_version='2', см. предупреждение выше) на
// момент обновления — 43 пары прогноз/факт. По ролям на тот момент:
// DEF_CENTRAL=12, DEF_WIDE=8, MID_CENTRAL=8, FWD_WIDE=5, MID_WIDE=4, GK=4,
// FWD_CENTRAL=2 — ни одна ещё не достигла MIN_CALIBRATION_SAMPLES, поэтому
// калибровка всё ещё ОБЩАЯ (не по ролям), не по каждой отдельно. Обычная
// линейная регрессия по всем 43 точкам дала slope≈0.595/intercept≈−0.380
// (R²≈0.77) — измеримо точнее предыдущей версии (33 точки/3 матча,
// slope≈0.52/intercept≈0.92, R²≈0.73) на ЭТИХ ЖЕ 43 точках: R² 0.699→0.767,
// средняя абсолютная ошибка (MAE) 1.089★→0.875★ (~20% ниже) — не просто шум
// от добавления данных, реальное улучшение по прежней выборке тоже.
// Отрицательный intercept — ожидаемо и не проблема: applyCalibration в
// zoneRatings.ts применяет пол
// STAR_SCALE_FLOOR=0.5 к результату, так что слабые игроки не уходят в
// минус, просто более круто "срезаются" вниз, чем раньше (что и было целью
// правки — старый хардкод завышал слабых и занижал сильных). Как только у
// роли накопится MIN_CALIBRATION_SAMPLES реальных матчей, настоящая
// регрессия в цикле ниже автоматически перекрывает эту заглушку для этой
// конкретной роли — удалять вручную ничего не придётся.
const PRELIMINARY_CALIBRATION: { slope: number; intercept: number } = { slope: 0.595, intercept: -0.38 };
