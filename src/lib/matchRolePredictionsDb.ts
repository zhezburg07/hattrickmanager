...
// регрессию по роли сама. Источник ТЕКУЩЕГО значения: полная выгрузка
// match_role_predictions (formula_version='2') на момент обновления — 43
// пары прогноз/факт. По ролям на тот момент: DEF_CENTRAL=12, DEF_WIDE=8,
// MID_CENTRAL=8, FWD_WIDE=5, MID_WIDE=4, GK=4, FWD_CENTRAL=2 — ни одна ещё
// не достигла MIN_CALIBRATION_SAMPLES, поэтому калибровка всё ещё ОБЩАЯ
// (не по ролям), не по каждой отдельно. Обычная линейная регрессия по всем
// 43 точкам дала slope≈0.595/intercept≈−0.380 (R²≈0.77) — измеримо точнее
// предыдущей версии (33 точки/3 матча, slope≈0.52/intercept≈0.92, R²≈0.73)
// на ЭТИХ ЖЕ 43 точках: R² 0.699→0.767, средняя абсолютная ошибка (MAE)
// 1.089★→0.875★ (~20% ниже) — не просто шум от добавления данных, реальное
// улучшение по прежней выборке тоже. Отрицательный intercept — ожидаемо и
// не проблема: applyCalibration в zoneRatings.ts применяет пол
