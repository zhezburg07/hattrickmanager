...
// specialty добавлена для воронки сбора данных калибровки тактик (см. чат
// "Автоматическая воронка сбора данных для будущей калибровки тактик") —
// аудит показал, что задним числом специализацию игрока на момент
// конкретного прошлого матча восстановить НЕЛЬЗЯ (это поле раньше не
// сохранялось вообще), а она нужна, чтобы в будущем отличить эффект
// командной тактики "Прессинг" от индивидуального эффекта "опорника"
// (Мощный + оборонительная позиция, см. официальный текст про стандарты/
// спецсобытия). Копится только ВПЕРЁД, с момента этого изменения — старые
// снимки её не содержат, задним числом не восстанавливается (специализация
// в Hattrick меняется очень редко, но не никогда).
...
// НЕ ИСПОЛЬЗУЕТСЯ СЕЙЧАС нигде в коде (см. чат "Унифицировать Потенциал с
// расчётом на слотах поля") — столбец "Потен." в "Составе"/"Расстановке"
// теперь считается через computePlayerPotential (zoneRatings.ts), той же
// формулой, что и число на слоте поля, с калибровкой по реальным звёздам.
// ЗАРЕЗЕРВИРОВАНО для будущего ИИ-помощника по трансферам (в дорожной
// карте, ещё не начат) — там понадобится быстрая оценка потенциала ЧУЖИХ
// игроков на трансферном рынке, для которых, в отличие от своего состава,
// никогда не будет полных данных (Loyalty/isClubProduct не приходят для
// чужих команд, а калибровка по match_role_predictions строится только по
// сыгранным матчам известных игроков) — эта более грубая, но ни от чего не
// зависящая оценка (только навыки + форма) там останется уместна. Не
// подключать никуда, пока эта фича не начата.
//
// Сама формула — сколько звёзд (0-10, та же шкала, что у Рейтинга последнего
