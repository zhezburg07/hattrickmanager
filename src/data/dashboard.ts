...
// (или её не удалось выполнить), используем KZT по умолчанию — основная
// аудитория проекта из Казахстана. Международное сокращение (KZT), а не
// полное слово "тенге" — по явному запросу пользователя (см. чат "Кубки/
// Юношеская команда/Трансферы: диагностика"), применяется везде одинаково
// (Финансы/Стадион/Трансферы — все через один и тот же currencyLabel).
export interface Currency {
  label: string; // подписывается рядом с суммами, например "2 458 900 KZT"
}

export const defaultCurrency: Currency = { label: "KZT" };
