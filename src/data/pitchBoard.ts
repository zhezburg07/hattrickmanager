...
//
// ВЫВЕДЕНО геометрически из FIELD_POSITIONS (см. MatchDetailAnalysis.tsx,
// x/y-координаты для отрисовки маркеров на поле, "схема 5-5-3") — САМО ПО
// СЕБЕ для целей калибровки НЕ подтверждено на живых данных. Перед тем как
// доверять этой таблице для записи в датасет калибровки, нужно сверить
// вручную с реальным отчётом матча на hattrick.org (кто на какой позиции
// стоял) для 2-3 матчей — временная диагностика в chppSync.ts (см. тот же
// чат) специально для этой проверки.
